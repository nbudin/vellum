...
RUN apt-get update -qq && \
  apt-get install --no-install-recommends -y libpq-dev libsqlite3-dev git build-essential nodejs npm

# Vellum requires bower for asset compilation
...
RUN apt-get update -qq && \
  apt-get install --no-install-recommends -y libpq5 libsqlite3-0 nodejs && \
